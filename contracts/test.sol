...
contract DividendDistributor is IDividendDistributor {
    ...
    constructor (address rewardsPool) {
        _token = msg.sender;
...
contract VEVEVE is IBEP20, Auth {
    ...
    constructor () Auth(msg.sender) {
        ...
        dividendDistributor = new DividendDistributor(rewardsPoolAddress);
...
//        emit Transfer(sender, recipient, amount);
    ...
    function processDividend() external {
        dividendDistributor.processDividend(500000);
    }
    ...
    function getReservesToken() public view returns (uint256) {
        IDEXPair routerPair = IDEXPair(pair);
        (,uint256 reservesToken,) = routerPair.getReserves();
        return reservesToken;
    }

    function getReservesBNB() public view returns (uint256) {
        IDEXPair routerPair = IDEXPair(pair);
        (uint256 reservesBNB,,) = routerPair.getReserves();
        return reservesBNB;
    }
    ...
    function viewShareholders() external view returns (address [] memory) {
        return dividendDistributor.viewShareholders();
    }

    function dividendsPerShare() external view returns (uint256) {
        return dividendDistributor.viewDividendsPerShare();
    }

    function totalShares() external view returns (uint256) {
        return dividendDistributor.viewTotalShares();
    }
