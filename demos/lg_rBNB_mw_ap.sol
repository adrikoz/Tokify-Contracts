...
//        emit Transfer(sender, recipient, amount);
    ...
    function getCurrentGapRatio(uint256 denominator) public view returns (uint256) {
        (uint256 reservesToken, uint256 reservesBNB) = router.getReserves(router.factory(), address(this), WBNB);
        uint256 gapRatio = denominator.mul((reservesToken.mul(getAmountBNBInAssetPool())).div(getCirculatingSupply().mul(reservesBNB)));
        return gapRatio;
    }
    ...
    function payoutFractionOfRewardsPool(uint256 numerator, uint256 denominator) external authorized {
        rewardsPool.transferBNBToDistributor(dividendDistributorAddress, rewardsPoolAddress.balance.mul(numerator).div(denominator));
    }
