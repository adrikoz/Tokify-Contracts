...
contract EverGrow is IBEP20, Auth {
    ...
    function sendCreatorWalletAmounts(uint256 amountBNBMarketing, uint256 amountBNBDeveloper, uint256 amountBNBTeam) internal {
        ...
        (bool developerTransferSuccess,) = address(developerFeeReceiver).call{value: developerAmount}("");
        require(developerTransferSuccess);
        /// @dev team wallet does not pay the generator tax to Tokify
        (bool teamTransferSuccess,) = address(teamFeeReceiver).call{value: amountBNBTeam}("");
        require(teamTransferSuccess);
